...
def test_operational_gateway_auth_error_to_dict() -> None:
    error = OperationalGatewayAuthError("login rejected")
    assert error.code == "OPERATIONAL_GATEWAY_AUTH_FAILED"
    assert error.details["reason"] == "login rejected"
    assert "operational Security Gateway" in error.message
