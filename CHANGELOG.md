...
### Fixed
- Restore reliable PDF chart rendering: upload SVG to MinIO and embed charts as data URIs in WeasyPrint (reverts presigned/download path that broke EC2 PDF generation).

## [2.2.9] - 2026-07-09

### Fixed
- CI Bandit failure: PDF charts use MinIO presigned URL generation plus SDK download (SAD step 8) instead of httpx with disabled TLS verification.
